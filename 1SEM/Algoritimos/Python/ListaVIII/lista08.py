...
# Solved by Fábio Lucas Romeiro (BDI - FATEC)
...
def near_ten(n):
    num = 10
    while n>=num or n%num <= 2 or n%num >= 8 and n%num <= 10:
        if n%num <= 2 or n%num >= 8 and n%num <= 10: return True
        else: num += 10
    return False
...
def lone_sum(a, b, c):
    numeros = []
    if a+b+c == a*3: return 0
    else:
        for num in [a,b,c]:
            if num in numeros:
                for i in range(len(numeros)):
                    if numeros[i] == num: numeros[i] = 0
...
def cat_dog(s):
    contCat, contDog = 0,0
    for i in range(len(s)-1):
        if s[i:i+3] == 'cat': contCat +=1
        if s[i:i+3] == 'dog': contDog +=1
    return contCat == contDog
...
def end_other(a, b):
    if len(a) >= len(b): return b.lower() in a.lower()[::-1][:len(b)][::-1]
    else: return a.lower() in b.lower()[::-1][:len(a)][::-1]
...
def fila_tijolos(n_peq, n_gra, meta):
    tamanhoG = 5
    qtdPequenos, qtdGrandes= n_peq, n_gra
    final = 0
    resto = meta%tamanhoG
    parametros = (meta-resto)/tamanhoG
    if(parametros > qtdGrandes):
        num = parametros - qtdGrandes
        qtdGrandes = 0
        final = final + (num * tamanhoG) + resto
    else:
        qtdGrandes = qtdGrandes - parametros
        final = resto
        if(final == 0): return True
    qtdPequenos = qtdPequenos - final
    if(qtdPequenos >= 0): return True
    else: return False
