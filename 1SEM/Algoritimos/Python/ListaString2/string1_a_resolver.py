#FABIO LUCAS ROMEIRO DE CASTRO - BDI
...
def donuts(n):
  num = str(n)
  if n>= 10 or n ==23: num = 'muitos'
  resp = 'Número de donuts: '+ num
  return resp
...
def pontas(s):
  resp = s[0:2] + s[len(s)-2] + s[len(s)-1]
  if len(s) < 2: resp = ''
  return resp 
...
def fixa_primeiro(s):
  ...
  for i in range(len(s)):
      if s[i] == primeiraLetra and i != 0: palavraCripto += '*'
      else: palavraCripto += s[i]
...
def busca(frase, palavra):
  ver = frase[:len(palavra)-1]
  return frase.count(ver)
