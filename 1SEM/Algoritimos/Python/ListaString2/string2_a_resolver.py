...
def verbing(s):
  resp = ''
  if len(s) >= 3 and s[::-1][0:3] != 'gni': resp = s +'ing'
  if s[::-1][0:3] == 'gni': resp = s + 'ly'
  if len(s)<3: resp = s
  return resp
...
def not_bad(s):
  bad = s.find('bad')
  nao = s.find('not')
  if nao != -1 and bad != -1 and bad > nao:
    return s.replace(s[nao:bad+3], 'good')
  return s
