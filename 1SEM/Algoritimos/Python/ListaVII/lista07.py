...
def maior_ponta(nums):
    if nums[0]>nums[-1]:maior = nums[0]
    else: maior = nums[-1]
    return [maior for i in range(len(nums))]
...
def sum2(nums):
    if len(nums)>1: return sum(nums[0:2])
...
def date_fashion(eu, par):
    ...
    elif eu > 2 and par >= 8 or eu > 8 and par > 2 or eu >= 8 and par >= 8: return 2
    elif eu < 8 and par < 8: return 1
...
def squirrel_play(temp, is_summer):
      ...
      elif is_summer and temp > 60 and temp<=100: return True
      else: return False
